...
int frame_process(Mat &bgrImg)
{
  ...
  isSuccess = markSensor->ProcessFrameLEDXYZ(bgrImg, angX, angY, Z,
                                             pix_x, pix_y);
...
public:
    ...
    : it_(nh_)
  {
    ...
    // Subscribe to the ros-dynamic-config
    ...
    ///200hz timer, actually we do not use it
  ...
  // Subscribe to ros dynamic config msg
  void cfg_cb(const ros_dynamic_test::dyn_cfgConstPtr &msg)
  {
    ...
    if(msg->is_red)
      ...
      markSensor->ap=AlgoriParam(msg->is_red,msg->ch1_min_b,msg->ch1_max_b,
                                 ...
                                 msg->ch3_max_b);
    //    MarkerParams::ifShow=msg->is_show_img;  // sometimes results in bugs
  }
  /// function in timer, not in use
  ...
  void process_frame()
  {
    if(is_windMill_mode)
    {
      ...
      // using algorithm written by yizhou
      ...
      // using algorithm written by zhangsheng
